...
async def _ensure_ingress(livekit_api: api.LiveKitAPI, pipeline: CameraPipeline) -> Any:
...
async def _probe_source_codec(camera: Camera, settings: Settings) -> str:
    command = [
        ...
        "-max_delay",
        str(settings.ingest_max_delay_us),
        "-reorder_queue_size",
        str(settings.ingest_reorder_queue_size),
...
async def _run_camera_loop(
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            ingress_info = await _ensure_ingress(livekit_api, pipeline)
...
async def run_ingest_service() -> None:
    ...
    pipelines = [
        CameraPipeline(
            ...
            ingress_name=f"{build_room_name(camera.id, settings.camera_room_prefix)}-ingress-{ingest_protocol}",
            participant_identity=f"ingress-{camera.id}",
